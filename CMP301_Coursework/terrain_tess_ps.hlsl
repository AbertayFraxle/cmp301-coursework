...
Texture2D normTex0 : register(t2);
Texture2D normTex1 : register(t3);


Texture2D depthMapTexture[LIGHTCOUNT] : register(t4);
...
float4 main(InputType input) : SV_TARGET
{
    ...
    if (input.worldPosition.y < TEXTURECHANGE)
    {
        if (diff < BLENDREGION)
        {
            ...
            newNorm = (normTex0.Sample(sampler0, input.tex) * bias1) + (normTex1.Sample(sampler0, input.tex) * bias2);
        ...
        else
        {
            ...
            newNorm = normTex0.Sample(sampler0, input.tex);
    ...
    else
    {
        ...
        newNorm = normTex1.Sample(sampler0, input.tex);
