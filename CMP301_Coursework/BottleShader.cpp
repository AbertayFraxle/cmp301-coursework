...
BottleShader::BottleShader(ID3D11Device* device, HWND hwnd) : BaseShader(device, hwnd)
{
	initShader(L"bottle_vs.cso", L"bottle_ps.cso");
}
...
void BottleShader::initShader(const wchar_t* vsFilename, const wchar_t* psFilename)
{
...
void BottleShader::setShaderParameters(ID3D11DeviceContext* deviceContext, const XMMATRIX& worldMatrix, const XMMATRIX& viewMatrix, const XMMATRIX& projectionMatrix, ID3D11ShaderResourceView* texture, ID3D11ShaderResourceView* normal, ShadowMap* shadowMap[LIGHTCOUNT], Light* lights[LIGHTCOUNT], Camera * camera)
{
	HRESULT result;
	...
	//pass all shadowmaps into the shader
	for (int i = 0; i < LIGHTCOUNT; i++) {
